...
- **Live weights** (read these from the code, not the spec): `approach=0.1`, `nail_driven=0.5`, `nail_depth_delta=600`, `impact_progress=8`, `completion=100`, `action_rate=−0.01`, `joint_pos_limits=−10`. `validate_rewards.py` reads them dynamically via `reward_manager.get_term_cfg(name).weight` — no need to keep its constants in sync.
...
- **Q1/Q2 are resolved** (V1 training, 2026-06-17; re-confirmed post-L6-fixture 2026-07-10): the single strike is feasible and `air_time_bonus` is not load-bearing (NOT added). After any EE/pose change (e.g. the 2026-07-06 L6 NEAR_NAIL re-solve), re-run `docs/research/reward-design/playback_reference.py` to re-confirm strike feasibility — the crude constant-vertical `docs/research/reward-design/test_single_strike.py` probe was **retired** (2026-07-10) after it failed from the new pose (pure-vertical descent is kinematically impossible with the L6 grasp near the floor); the script still exists but no longer serves as the feasibility gate.
