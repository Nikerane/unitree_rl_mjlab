"""Shell contract for the frozen matched impulse-CaT p=0.2 bridge evaluation."""
...
LAUNCHER = ROOT / "scripts/slurm/vega_vic_impulse_p02_bridge_eval.sbatch"
EXPECTED_OUTPUTS = (
  "fixed_trace.npz",
...
TARGET_SHA = "57000e958bbafa2c62929652d3b76fd6ed571c9867bee3735c14baf0ca57d8de"
...
def _git(path: Path, *args: str) -> str:
  result = subprocess.run(
    [
      "git",
      "-c",
      "user.email=p02-bridge-eval-test@example.com",
      "-c",
      "user.name=P02 Bridge Eval Test",
      *args,
    ],
    cwd=path,
    check=True,
...
def _env(tmp_path: Path, task_id: int) -> dict[str, str]:
  ...
  control, control_file_sha = _checkpoint(
    tmp_path / "checkpoints/control/model_499.pt", b"bridge control checkpoint\n"
  )
  target, target_file_sha = _checkpoint(
    tmp_path / "checkpoints/target/model_499.pt", b"bridge p02 checkpoint\n"
  )
  ...
  env = {
    ...
    "INITIAL_CONTROL_FILE_SHA": control_file_sha,
    "INITIAL_TARGET_FILE_SHA": target_file_sha,
    "REAL_GIT": real_git,
...
def test_launcher_maps_exact_bridge_roles_hashes_and_fresh_five_artifact_leaves(
  tmp_path: Path,
):
  assert EVALUATION_CHECKPOINTS["bridge_p0_control"] == CONTROL_SHA
  assert EVALUATION_CHECKPOINTS["bridge_p02_target"] == TARGET_SHA
  for task_id, role, checkpoint_key in (
    (0, "bridge_p0_control", "CONTROL_CHECKPOINT"),
    (1, "bridge_p02_target", "TARGET_CHECKPOINT"),
  ):
    ...
    assert output.parent.name == f"24680_{task_id}_{role}"
...
def test_launcher_refuses_collisions_bad_hashes_drift_and_extra_outputs(tmp_path: Path):
  env = _env(tmp_path / "collision", 0)
  result_root = (
    Path(env["HOME"])
    / "campaigns/z1-vic-impulse-p02-bridge/evaluation"
    / env["EXPECTED_CODE_REVISION"]
    / env["EXPECTED_ASSET_REVISION"]
  )
  (result_root / "24680_0_bridge_p0_control").mkdir(parents=True)
  result = _run(env)
  assert result.returncode == 2
  assert "attempt directory already exists" in result.stdout

  env = _env(tmp_path / "bad-hash", 1)
  env["TARGET_SHA"] = "0" * 64
  result = _run(env)
  assert result.returncode == 2
  assert "role/checkpoint SHA-256 mismatch" in result.stdout

  env = _env(tmp_path / "drift", 1)
  env["MUTATE_CHECKPOINT"] = env["TARGET_CHECKPOINT"]
  result = _run(env)
  assert result.returncode == 2
  assert "checkpoint SHA-256 drifted during evaluation" in result.stdout

  env = _env(tmp_path / "extra", 0)
  env["EXTRA_HIDDEN_OUTPUT"] = "1"
  result = _run(env)
  assert result.returncode == 2
  assert "exactly five artifacts" in result.stdout


def test_launcher_refuses_dirty_repositories_and_symlink_checkpoint(tmp_path: Path):
...
def test_launcher_freezes_two_a100_arms_time_and_zero_retry_contract():
