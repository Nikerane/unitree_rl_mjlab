...
def _initial_episode_joint_peaks(trace: Mapping[str, np.ndarray]) -> np.ndarray:
  lam = np.asarray(trace["lambda_per_joint"], dtype=np.float64)
  episode_id = np.asarray(trace["episode_id"], dtype=np.int64)
  valid = episode_id == 0
  if lam.ndim != 3 or episode_id.shape != lam.shape[:2] or not valid.any(axis=0).all():
    raise ValueError("trace lacks aligned initial-episode Lambda reads")
  return np.where(valid[:, :, None], lam, -np.inf).max(axis=0)
...
def _first_contact_event_utilization(
  trace: Mapping[str, np.ndarray], caps: Sequence[float]
) -> dict[str, object]:
  lam = np.asarray(trace["lambda_per_joint"], dtype=np.float64)
  episode_id = np.asarray(trace["episode_id"], dtype=np.int64)
  ...
  return {
    "scope": "observed control reads overlapping the first physical-contact-event prefix",
    "segments_with_contact": len(values),
  ...
  for joint in range(6):
    ...
    for timing in ("precontact", "at_contact"):
      for field in ("vic_p", "vic_kp", "vic_kd"):
        ...
        joint_summary[f"{field}_{timing}"] = _quantiles(np.asarray(values))
...
def _pressure_split(
  ...
  *,
  ...
  return {
    key: {"windows": len(values), "event_pressure": _quantiles(np.asarray(values))}
...
def _compact_threshold(
  ...
  *,
  ...
  return {
    ...
    "first_contact_event_utilization": _first_contact_event_utilization(trace, caps),
    ...
    "offline_cat_at_p_0_5": {
      ...
      "event_pressure_by_physical_censoring_and_ambiguity": _pressure_split(
...
def analyze_result_root(root: Path) -> dict[str, object]:
  ...
  return {
    ...
    "provenance": {
      ...
      "scheduler_status_from_authoritative_task_8_brief": {
      ...
      "validated_input_artifacts": manifests,
