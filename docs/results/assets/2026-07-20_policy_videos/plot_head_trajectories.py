...
zlim = (min(allz)-0.01, max(allz+[nail_z])+0.02)
...
for ax, label in zip(axes.flat, order):
    ...
    # swing excursion
    fwd = max(p[0] for p in a["substep"]) - nail_x
...
labs = sorted(order, key=lambda l: max(p[0] for p in arms[l]["substep"]) - nail_x)
fwd = [(max(p[0] for p in arms[l]["substep"]) - nail_x) * 100 for l in labs]
