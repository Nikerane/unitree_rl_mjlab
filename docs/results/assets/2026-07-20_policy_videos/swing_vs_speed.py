...
for label, a in arms.items():
    sub = a["substep"]  # [x,y,z,contact]
    fc = next((i for i, p in enumerate(sub) if p[3]), None)
    if fc is None or fc < 1:
        rows.append((label, a["max_nail_mm"], float("nan"), float("nan"), None)); continue
    ...
    rows.append((label, a["max_nail_mm"], swing_pre * 100, v_touch, fc))

rows.sort(key=lambda r: (-(r[3] if r[3] == r[3] else -1)))
print(f"{'policy':13s} {'depth_mm':>8} {'swing_pre_cm':>12} {'v_touch_m/s':>11} {'contact@substep':>15}")
for label, depth, sw, vt, fc in rows:
    print(f"{label:13s} {depth:8.1f} {sw:12.1f} {vt:11.3f} {str(fc):>15}")

# scatter: pre-contact swing vs contact speed (drivers = drove >=20mm), labelled
good = [(l, sw, vt, depth) for (l, depth, sw, vt, fc) in rows if vt == vt and depth >= 20]
weak = [(l, sw, vt, depth) for (l, depth, sw, vt, fc) in rows if vt == vt and depth < 20]
...
for (l, sw, vt, depth) in weak:
    ...
    ax.annotate(f"{l} ({depth:.0f}mm)", (sw, vt), fontsize=7, color="#888",
...
ax.set_title("Does the swing buy impact speed?\n"
             "blue/orange = drove nail ≥20mm · grey = under-driver   "
