...
OUT = "/private/tmp/claude-501/-Users-nikerane-repos-unitree-rl-mjlab/2cd822e7-752b-4070-969b-f93a7bd62f87/scratchpad/dc_multireset.json"
I_REF = 0.6094; N = 30; BASE = 1000
ARMS = [(f"{lab}_s{s}", f"*_{pat}_seed{s}") for lab, pat in
        [("maxoff","mx_maxoff"),("imponly","dc_imponly"),("delonly","dc_delonly"),("maxmax","mx_maxmax")]
        for s in (0, 1, 2)]
...
for label, prefix in ARMS:
    dirs = sorted(glob.glob(f"{ROOT}/{prefix}")); ck = sorted(glob.glob(f"{dirs[0]}/model_*.pt") if dirs else [], key=lambda p: int(p.split("model_")[1].split(".pt")[0]))
