...
- C1 delivered-side flicker-press re-arm (`_event_age` resets every rising edge) — still present.
  **UPDATE 2026-07-14: FIXED** — re-arm now debounced (`rearm_gap_substeps`, default = window; unit-tested).
- Adversarial-review fixes **I3–I12 + minors** — deferred (metric-mean bias I3, derive-doc falsified headroom I4, tautology gate I5, non-durable C2 provenance I9, `test_contact_row_impulse.py:329` falsified invariant I10, no joint-order test I11, no eval-snapshot test I12, …).
  **UPDATE 2026-07-14: I3–I12 ALL FIXED** (housekeeping batch): `delivered_total` full-step metric (I3), derive-script headroom re-framing (I4), tautology gate demoted to informational (I5), `substep_impulse_rows` `enabled` perf flag (I6), docs/README de-staled (I7), IMPULSE_CAT_IMPL_PLAN status refreshed (I8), durable C2 record `docs/results/2026-07-10_c2_enforcement_record.md` (I9), falsified `rows ≤ 1.05×raw` test replaced with the rows≈sub convergence invariant (I10), end-to-end joint-order pin test (I11), committed pre-reset snapshot-hook positive control `tests/test_eval_impulse_hook.py` (I12). NOTE: the batch landed across two commits — the I4/I5 derive-script edits, `env_cfgs.py`, and the I10/I11 test rework rode into the concurrent R2 review-fix commit (`96ba1e1`, whose gate runs validated them: C0 QUANTITY GATE PASS under the demoted-[6] + real-sweep semantics); the remainder (accumulator debounce, `delivered_impulse_total`, `enabled` flag, I12 test, docs) banked in the follow-up housekeeping commit. The "minors" list (normalizer merge, consume(), playback-idiom extraction, depth-gate triplication, scratch-buffer allocs) remains deferred to pre-C5/VIC.
