...
- **Real Z1 joint-velocity limit = 3.1415 rad/s** (official `z1_description` URDF), torque 30/60 N·m (already matched in sim). ⚠️ **CORRECTION (during impl):** `max_dq` is NOT a clean velocity limit and is NOT `3.1415·dt` — it clamps the per-substep IK increment and the PD settles at `qvel ≈ (kp/kd)·max_dq` (kp/kd=10), so `0.0063` craters the arm. **`delta_pos_scale=0.15` is self-limiting (peak 2.41 rad/s < 3.1415) so no rail is needed at this scale**; if ever raised toward ~2.5 m/s, a calibrated rail is `max_dq ≈ 0.29–0.31`. Task 2 below is superseded by this finding (decision: rely on self-limiting + rollout monitoring, pending).
