...
Pin the efc→joint mapping empirically; lock it with `Σ_all_rows (JᵀF) == qfrc_constraint`. **Known API facts (encode, do not rediscover):** active contact count is `d.nacon` (NOT `ncon`; `contact.geom.shape[0]` is allocated capacity `naconmax`); the efc struct has NO `worldid` (fields: `type,id,J_rownnz,J_rowadr,J_colind,J,pos,margin,D,vel,aref,frictionloss,force,state,Ma,Jqvel`); `d.contact` HAS `worldid`; compiled geom names are **entity-prefixed**: `robot/hammer_head_0`, `robot/hammer_head_1`, `nail_block/nail_shaft`, `nail_block/nail_head` (exclude `nail_block/block_geom`).

**MJWarp caveats from the 2026-07-10 qfrc audit (`docs/research/reward-design/QFRC_MEASUREMENT_AUDIT.md` §2.2 — treat as probe hypotheses, the invariant test adjudicates):** the MJWarp docs state `efc.J` is stored **DENSE and padded** (sparse "not implemented") even though the sparse-index fields exist in the dataclass — probe which path is live and never read padding rows; **row ordering on-device is NOT C MuJoCo's** (host readback re-sorts) — index through `contact.efc_address`/`contact.dim` and classify by `efc.type`, never address arithmetic over the whole array; **silent truncation**: assert `nefc < njmax` and `nacon < naconmax` in the helper (an under-sized allocation silently drops rows); MJWarp computes **float32** and GPU runs are non-deterministic — the invariant tolerance (rtol 1e-4) is float32-appropriate on CPU, never bit-compare across runs.
...
- [ ] **Step 1: Section [6]** — per reference strike print `raw Λ | baseline-subtracted Λ | contact-row Λ | object-side ∫F·dt` per joint + derived shares (`friction share = (raw − rows)/raw`, `residual after subtraction = (subtracted − rows)/rows`); save a grouped per-joint bar chart (`matplotlib.use("Agg")`). **Hard gate (AMENDED 2026-07-10):** `rows ≤ raw + noncontact` (5% tolerance + small atol), where `noncontact_j = Σ|qfrc_constraint_j − contact_rows_j|·dt` over the same window. *Amendment rationale:* the original `rows ≤ raw` bound assumed same-sign contamination and was empirically falsified on joint3 (raw 0.0146 < rows 0.0420 — dof-friction OPPOSES the contact reaction, so `|raw| = |contact + friction| < |contact|`; reproduced independently by Phase M5 on a straight-down strike). The amended bound is the sign-aware triangle-inequality form (`|contact| = |qfrc − (qfrc − contact)| ≤ |qfrc| + |qfrc − contact|`, exact per substep) — a true theorem given the Task-8 reconstruction invariant that still catches attribution bugs (double-counted rows, wrong-world reads). Keep the per-joint `rows/raw` ratio printed as informative output: the joint3 cancellation is a thesis-relevant finding (raw undershoots the true reaction; validates `subtract_baseline=True` and `rows` as ground truth). **Failure path:** if contact-row Λ disagrees with the object-side ∫F·dt beyond the friction-residue band, print `[TRACK-2 BUG]` — blocks Tasks 8–10 conclusions + any promotion decision, NEVER Track 1; record it in Step 4's note.
- [ ] **Step 2: Phase M extension** — rows accumulator (wired or manually instantiated) zero before contact, positive after the scripted strike, ≤ raw + noncontact (same amended sign-aware bound as Step 1).
