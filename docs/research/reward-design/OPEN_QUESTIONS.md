...
1. ~~The press is now physically excluded~~ **CORRECTED same day (validate_rewards Phase J + playback press probe):** the 20.5 mm "stall" was an artifact of measuring the press only from *post-lift* arm postures. From the **reset pose**, a sustained press pushes through 30 N friction and reaches the 30 mm threshold in ~11 control steps. **The press exploit persists at calibrated friction** — this is Path-A outcome #2 (quasi-static solutions survive on a stiff-PD position action space), i.e. direct empirical justification for the thesis's variable-impedance architecture. Press exclusion is therefore the *reward design's* job (T3 one-payout impact window + time penalty + T2 reference prior), with the press-watchdog metric load-bearing in every training run.
