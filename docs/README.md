...
| `docs/superpowers/specs/2026-08-18-z1-impulse-cat-thesis-campaign-design.md` | Approved staged scientific design: complete-contact shadow, offline dose gate, one-seed bridge, then eight-seed provisional-cap confirmation; design only, with every stage separately authorized |
| `docs/research/reward-design/Z1_IMPULSE_CAT_CAMPAIGN_CROSS_MODEL_PACKET.md` | Sanitized aggregate-only packet for independent Kimi/GLM/DeepSeek/Qwen design critique; no review has been sent or fabricated |
...
- 2026-08-18 — the next thesis campaign was frozen as a gated sequence rather than a training sweep: complete-contact release/window-flush shadow first, offline event-dose selection second, one provisional-cap bridge arm third, and only then eight paired PPO seeds if impulse, velocity, and utility gates all pass. The design authorizes no run (`docs/superpowers/specs/2026-08-18-z1-impulse-cat-thesis-campaign-design.md`)
